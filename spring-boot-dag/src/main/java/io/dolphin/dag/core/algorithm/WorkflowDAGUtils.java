...
public class WorkflowDAGUtils {
    ...
    public static boolean valid(PEWorkflowDAG peWorkflowDAG) {
        if (peWorkflowDAG == null || peWorkflowDAG.getNodes() == null || peWorkflowDAG.getEdges() == null) {
            return false;
        }
        // 1. 检查节点是否重复
    ...
    public static WorkflowDAG convert(PEWorkflowDAG peWorkflowDAG) {
        ...
        if (peWorkflowDAG.getNodes() == null || peWorkflowDAG.getNodes().isEmpty()) {
            throw new DolphinException("WorkflowDAGUtils.convert: nodes is empty graph");
        }
        
        // 创建节点
        peWorkflowDAG.getNodes().forEach(dotLineNode -> {
            Long nodeId = dotLineNode.getNodeId();
            WorkflowDAG.Node referenceNode = new WorkflowDAG.Node(Lists.newLinkedList(), nodeId, dotLineNode.getJobId(), dotLineNode.getNodeName(), InstanceStatus.WAITING_DISPATCH.getV());
            id2Node.put(nodeId, referenceNode);
            // 初始阶段，每一个点都设为顶点
            rootIds.add(nodeId);
        });
        ...
        peWorkflowDAG.getEdges().forEach(edge -> {
            ...
            if (from == null || to == null) {
                throw new DolphinException("WorkflowDAGUtils.convert: edge is invalid");
            }
        ...
        // 合法性校验（至少存在一个顶点）
        if (rootIds.isEmpty()) {
            throw new DolphinException("Illegal DAG: " + JsonUtils.toJSONString(peWorkflowDAG));
        }

        List<WorkflowDAG.Node> roots = Lists.newLinkedList();
        rootIds.forEach(rootId -> roots.add(id2Node.get(rootId)));
    ...
    public static List<PEWorkflowDAG.Node> listReadyNodes(PEWorkflowDAG dag) {
        ...
        dag.getNodes().forEach(node -> nodeId2Node.put(node.getNodeId(), node));
