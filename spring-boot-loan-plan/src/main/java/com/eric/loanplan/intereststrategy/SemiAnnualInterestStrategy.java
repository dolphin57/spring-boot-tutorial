...
public class SemiAnnualInterestStrategy implements InterestCalculationStrategy {
    @Override
    public Map<Integer, BigDecimal> calculateInterest(BigDecimal loanAmount, BigDecimal annualInterestRate, int loanTerm) {
        ...
        for (int i = 1; i <= loanTerm; i++) {
            BigDecimal perInterest = loanAmount.multiply(annualInterestRate).divide(BigDecimal.valueOf(2), 2, BigDecimal.ROUND_HALF_UP);
